...
plt.ylabel("RF Transmitter Frequency [dB]")
plt.colorbar()
plt.show()
...
for i in power:
    if (i == prev_power) and (i>-18 and abs(freq[indexs]-prev_freq)>100):
        count += 1
        sum_tot += abs(freq[indexs]-prev_freq)
        prev_freq = freq[indexs]
    elif i == prev_power and i>-18 and abs(freq[indexs]-prev_freq)>800:
        count +=0
    else:
        if count != 0:
...
plt.plot(x2, y2, 'r')
plt.xlabel("RF Transmitter Power [dB]")
plt.ylabel("Avg freq step detected Shapiro steps [MHz]")
